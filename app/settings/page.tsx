...
export default function SettingsPage() {
  ...
  const deleteApiKey = async (service: ApiKeyService) => {
    ...
    try {
      const response = await fetch(
        `/api/settings/api-keys?service=${service}`,
        {
          method: "DELETE",
          credentials: "include",
        },
  ...
  const toggleShowPassword = (service: ApiKeyService) => {
    setApiKeyStates((prev) => ({
      ...
      [service]: {
        ...prev[service],
        showPassword: !prev[service].showPassword,
      },
    }));
  };
  ...
  const getServiceInfo = (service: ApiKeyService) => {
    const info: Record<
      ApiKeyService,
      { description: string; url: string; urlLabel: string }
    > = {
      supadata: {
        description: "Fetch YouTube transcripts with timestamps",
        url: "https://supadata.ai/?ref=jeremiasaguero0322",
                ...
                <div className="flex gap-3">
                  <div className="relative flex-1">
                    <Input
                      ...
                      value={state.value}
                      onChange={(e) =>
                        updateApiKeyValue(service, e.target.value)
                      }
                      placeholder={
                        keyInfo?.configured
          ...
          <motion.div variants={itemVariants}>
            <GlassCard variant="elevated">
              <GlassCardHeader className="px-6 py-6">
                ...
                <GlassCardDescription>
                  Manage your API keys for transcript fetching and AI models.
            ...
            <GlassCard variant="elevated" className="border-red-200">
              <GlassCardHeader className="px-6 py-6">
                <div className="flex items-center gap-2">
                  ...
                  <GlassCardTitle className="text-red-600">
                    Danger Zone
                  </GlassCardTitle>
                </div>
                <GlassCardDescription>
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div>
                      ...
                      <p className="text-sm text-red-600 mt-1">
                        ...
                        including summaries, API keys, and settings.
                      </p>
                    </div>
                    {!showDeleteConfirm ? (
                      <Button
